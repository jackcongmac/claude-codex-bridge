...
def _prompt(self_name, msgs, prompt_msg, agents=None):
    ...
    return (
        ...
        "FORMAT: the chat renders markdown, so when your reply has multiple points use "
        "`- ` bullets, `**bold**` for key terms, `1.` for ordered steps, and short lines — "
        "keep it scannable. For a one-liner just write the sentence (no bullets).\n\n"
